...
class DiscretionaryPowerAnalyzer:
    ...
    def _format_markdown_report(self, results: Dict[str, Any]) -> str:
        ...
        for risk_level in ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW']:
            ...
            # CRITICAL and HIGH: Show ALL findings (publication-ready accountability)
            if risk_level in ['CRITICAL', 'HIGH']:
            ...
            # MEDIUM and LOW: Show summary table + reference index (readability)
            else:
                # Show first 5 examples
                md += "#### Representative Examples (First 5)\n\n"
                for i, finding in enumerate(level_findings[:5], 1):
                    md += f"**{i}. {finding['pattern_type'].replace('_', ' ').title()}**\n\n"
                    md += f"- **Location:** {finding['location']['section']} "
                    md += f"({finding['location']['position']}% through document)\n"
                    md += f"- **Matched Text:** `{finding['matched_text']}`\n"
                    md += f"- **Context:** {finding['context'][:200]}...\n"
                    md += f"- **Assessment:** {finding['risk_assessment']}\n\n"
                
                # Generate location reference index
                md += f"#### Complete Reference Index ({len(level_findings)} findings)\n\n"
                ...
                for finding in level_findings:
                    ...
                    pattern = finding['pattern_type'].replace('_', ' ').title()
                    matched = finding['matched_text']
                    by_section[section].append(f"{pattern}: `{matched}`")
                
                # Sort sections by first appearance
                sorted_sections = sorted(by_section.items(), 
                                       key=lambda x: level_findings[[f['location']['section'] for f in level_findings].index(x[0])].get('location', {}).get('position', 0))
                
                for section, patterns in sorted_sections:
                    md += f"**{section}**\n"
                    for pattern in patterns[:10]:  # Max 10 per section
                        md += f"- {pattern}\n"
                    if len(patterns) > 10:
                        md += f"- *... and {len(patterns) - 10} more in this section*\n"
                    md += "\n"
                
                # Pattern frequency summary
                md += "#### Pattern Frequency Summary\n\n"
                pattern_counts = defaultdict(int)
                for finding in level_findings:
                    pattern_counts[finding['pattern_type']] += 1
                
                md += "| Pattern Type | Count | % of Total |\n"
                md += "|--------------|-------|------------|\n"
                for pattern, count in sorted(pattern_counts.items(), key=lambda x: x[1], reverse=True):
                    pct = (count / len(level_findings)) * 100
                    md += f"| {pattern.replace('_', ' ').title()} | {count} | {pct:.1f}% |\n"
                md += "\n"
                
                md += f"*Full details for all {len(level_findings)} findings available in JSON export.*\n\n"
